...
bool GlobalActor::PixelCollisionMapUpdate(GlobalActor* _Actor)
{
	...
	float4 Color = MapTexture_->GetPixel(static_cast<float>(dynamic_cast<GlobalActor*>(_Actor)->GetTransform().GetWorldPosition().ix()), -(static_cast<float>((dynamic_cast<GlobalActor*>(_Actor)->GetTransform().GetWorldPosition().iy()) + 39.f)));
